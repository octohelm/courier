...
使用者项目应在模型目录（如 `pkg/apis/{domain}/{vMajor}`）承载公开契约里的校验与文本表示规则。

## validate 放置位置

优先把字段级校验规则定义在使用者项目的模型上（典型路径 `pkg/apis/{domain}/{vMajor}`）。

常见写法：

- 字段 tag 使用 `validate:"..."`
- 校验引擎和内置规则见 `go doc github.com/octohelm/courier/pkg/validator`
...
这类能力会被 validator 包识别（详见 `go doc github.com/octohelm/courier/pkg/validator`）。
...
- 自定义文本类型可复用 content 包处理请求编解码（详见 `go doc github.com/octohelm/courier/pkg/content`）
