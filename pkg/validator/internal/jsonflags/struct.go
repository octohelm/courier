...
func (f *StructField) GetOrNew(v reflect.Value) reflect.Value {
	if len(f.index) == 1 {
...
var StructVisitor = &structVisitor{}
...
func (v *structVisitor) StructFields(typ reflect.Type) (*StructFields, error) {
	if vv, ok := v.cache.Load(typ); ok {
...
func makeStructFields(root reflect.Type) (*StructFields, *json.SemanticError) {
	...
	sfs := &StructFields{
		flattened:    allFields,
		byActualName: make(map[string]*StructField, len(allFields)),
		byFoldedName: make(map[string]*StructField, len(allFields)),
	}

	for _, f := range sfs.flattened {
		sfs.byActualName[f.Name] = f
		sfs.byFoldedName[string(foldName([]byte(f.Name)))] = f
	}
