...
func TestStringValidator_Validate(t *testing.T) {
	...
	cases := []struct {
		...
		{[]any{reflect.ValueOf("a"), String("aa"), "aaa", "aaaa", "aaaaa"}, &StringValidator{
			MaxLength: ptr.Ptr[uint64](5),
		}, "less than"},
		{[]any{"一", "一一", "一一一"}, &StringValidator{
			LenMode:   StrLenModeRuneCount,
			MaxLength: ptr.Ptr[uint64](3),
		}, "char count less than"},
		{[]any{"A", "B"}, &StringValidator{
			Enums: []string{
				"A",
				"B",
			},
		}, "in enum"},
		{[]any{"word", "word1"}, &StringValidator{
			Pattern: `^\w+`,
		}, "regexp matched"},
	}
...
func TestStringValidator_ValidateFailed(t *testing.T) {
	...
	cases := []struct {
		...
		{[]any{1.1, reflect.ValueOf(1.1)}, &StringValidator{
			MinLength: 5,
		}, "unsupported types"},
		{[]any{"a", "aa", String("aaa"), []byte("aaaa")}, &StringValidator{
			MinLength: 5,
		}, "too small"},
		{[]any{"aa", "aaa", "aaaa", "aaaaa"}, &StringValidator{
			MaxLength: ptr.Ptr[uint64](1),
		}, "too large"},
		{[]any{"字符太多"}, &StringValidator{
			LenMode:   StrLenModeRuneCount,
			MaxLength: ptr.Ptr[uint64](3),
		}, "too many chars"},
	}
