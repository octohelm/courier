...
func (validator *StructValidator) Validate(v any) error {
	switch rv := v.(type) {
	case reflect.Value:
...
func (validator *StructValidator) ValidateReflectValue(rv reflect.Value) error {
	return validator.validate(rv)
}

func (validator *StructValidator) validate(rv reflect.Value, path ...any) error {
	typ := rv.Type()
