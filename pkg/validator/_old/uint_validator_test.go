...
func TestUintValidator_Validate(t *testing.T) {
	cases := []struct {
		...
		{[]any{reflect.ValueOf(uint(1)), uint(2), uint(3)}, &UintValidator{
			Enums: []uint64{
				1,
				2,
				3,
		...
		{[]any{uint(2), uint(3), uint(4)}, &UintValidator{
			Minimum: 2,
			Maximum: 4,
		}, "in range"},
		{[]any{uint8(2), uint16(3), uint32(4), uint64(4)}, &UintValidator{
			Minimum: 2,
			Maximum: 4,
		}, "uint types"},
		{[]any{uint64(2), uint64(3), uint64(4)}, &UintValidator{
			BitSize: 64,
			Minimum: 2,
			Maximum: 4,
		}, "in range"},
		{[]any{uint(2), uint(4), uint(6)}, &UintValidator{
			MultipleOf: 2,
		}, "multiple of"},
	}
...
func TestUintValidator_ValidateFailed(t *testing.T) {
	cases := []struct {
		...
		{[]any{2, "string", reflect.ValueOf(1)}, &UintValidator{
			BitSize: 64,
		}, "unsupported type"},
		{[]any{uint(4), uint(5), uint(6)}, &UintValidator{
			Enums: []uint64{
				1,
				2,
				3,
		...
		{[]any{uint(1), uint(4), uint(5)}, &UintValidator{
			Minimum:          2,
			Maximum:          4,
			ExclusiveMaximum: true,
		}, "not in range"},
		{[]any{uint(1), uint(3), uint(5)}, &UintValidator{
			MultipleOf: 2,
		}, "not multiple of"},
	}
