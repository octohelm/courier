...
func (validator *MapValidator) Validate(v any) error {
	switch rv := v.(type) {
	case reflect.Value:
	...
	if validator.KeyValidator != nil || validator.ElemValidator != nil {
		...
		for _, key := range rv.MapKeys() {
			...
			if validator.KeyValidator != nil {
				...
				if err != nil {
					finalErr = statuserror.Join(
						finalErr,
						statuserror.Parameter(err, "", fmt.Sprintf("%v/key", vOfKey)),
					)
				}
			}
			if validator.ElemValidator != nil {
				...
				if err != nil {
					finalErr = statuserror.Join(
						finalErr,
						statuserror.Parameter(err, "", fmt.Sprintf("%v", vOfKey)),
					)
				}
			}
		}
