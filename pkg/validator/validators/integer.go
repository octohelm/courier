...
func init() {
	internal.Register(&intValidatorCreator{})
}

type intValidatorCreator struct {
}

func (c *intValidatorCreator) Validator(rule *rules.Rule) (internal.Validator, error) {
...
func (c *intValidatorCreator) readBitSize(rule *rules.Rule) (uint, bool, error) {
	unsigned := false
	...
	if bitSizeBuf.Len() != 0 {
		...
		if err != nil || bitSizeNum > 64 {
			return 0, false, rules.NewSyntaxError("int parameter should be valid bit size, but got `%s`", bitSizeStr)
		}
...
func (intValidatorCreator) Names() []string {
	return []string{
...
func (validator *IntegerValidator[T]) Validate(value jsontext.Value) (jsontext.Value, error) {
	if value.Kind() != '0' {
		return value, fmt.Errorf("unsupported type")
	}
	...
	switch any(val).(type) {
	case uint64:
		v, err := strconv.ParseUint(string(value), 10, int(validator.BitSize))
		if err != nil {
			return nil, fmt.Errorf("invalid integer value %w", err)
		}
		val = T(v)
	case int64:
		v, err := strconv.ParseInt(string(value), 10, int(validator.BitSize))
		if err != nil {
			return nil, fmt.Errorf("invalid integer value %w", err)
		}
		val = T(v)
	...
	if validator.Enums != nil {
		...
		if !in {
			return value, &validatorerrors.NotInEnumError{
				Target:  "int value",
				Current: val,
				Enums:   enums,
			}
	...
	if ((validator.ExclusiveMinimum && val == mininum) || val < mininum) ||
		...
		return value, &validatorerrors.OutOfRangeError{
			Target:           "int value",
			Current:          val,
			Minimum:          mininum,
	...
	if validator.MultipleOf != 0 {
		if val%validator.MultipleOf != 0 {
			return value, &validatorerrors.MultipleOfError{
				Target:     "int value",
				Current:    val,
