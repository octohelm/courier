...
func TestIntegerValidatorProvider(t *testing.T) {
	...
	for _, r := range rules {
		t.Run("parse "+r[0], func(t *testing.T) {
			Then(t, "integer validator 规则会被规范化", ExpectMust(func() error {
...
func TestIntegerValidator(t *testing.T) {
	t.Run("accepts valid integer input", func(t *testing.T) {
	...
	t.Run("rejects invalid integer input", func(t *testing.T) {
		cases := testutil.Cases{
			{
				Input: []byte(`{}`),
				Target: &struct {
