	...
	caseSet := map[reflect.Type][]struct {
		...
		reflect.TypeOf(map[string]map[string]string{}): {
			{"@map<,@map[1,2]>[1,]", &MapValidator{
				MinProperties: 1,
				ElemValidator: MustCompile(context.Background(), []byte("@map[1,2]"), typesutil.FromRType(reflect.TypeOf(map[string]string{})), nil),
			}},
			{"@map<@string[0,],@map[1,2]>[1,]", &MapValidator{
				MinProperties: 1,
				KeyValidator:  MustCompile(context.Background(), []byte("@string[0,]"), typesutil.FromRType(reflect.TypeOf("")), nil),
				ElemValidator: MustCompile(context.Background(), []byte("@map[1,2]"), typesutil.FromRType(reflect.TypeOf(map[string]string{})), nil),
			}},
		},
	}
	...
	for typ, cases := range caseSet {
		for _, c := range cases {
			t.Run(fmt.Sprintf("%s %s|%s", typ, c.rule, c.expect.String()), func(t *testing.T) {
				v, err := c.expect.New(context.Background(), MustParseRuleStringWithType(c.rule, typesutil.FromRType(typ)))
	...
	for typ := range invalidRules {
		for _, r := range invalidRules[typ] {
			...
			t.Run(fmt.Sprintf("validate %s new failed: %s", typ, rule.Bytes()), func(t *testing.T) {
				_, err := validator.New(context.Background(), rule)
...
func TestMapValidator_Validate(t *testing.T) {
	cases := []struct {
		...
		{[]interface{}{
			...
			MinProperties: 2,
			MaxProperties: ptr.Ptr[uint64](4),
			KeyValidator:  MustCompile(context.Background(), []byte("@string[1,]"), typesutil.FromRType(reflect.TypeOf("1")), nil),
			ElemValidator: MustCompile(context.Background(), []byte("@string[1,]?"), typesutil.FromRType(reflect.TypeOf("1")), nil),
		}, "key value validate"},
	}
...
func TestMapValidator_ValidateFailed(t *testing.T) {
	cases := []struct {
		...
		{[]interface{}{
			...
			MinProperties: 2,
			MaxProperties: ptr.Ptr[uint64](4),
			KeyValidator:  MustCompile(context.Background(), []byte("@string[2,]"), typesutil.FromRType(reflect.TypeOf("")), nil),
			ElemValidator: MustCompile(context.Background(), []byte("@string[2,]"), typesutil.FromRType(reflect.TypeOf("")), nil),
