...
func TestXMLTransformer(t *testing.T) {
	...
	ct, _ := transformer.NewTransformer(context.Background(), typesutil.FromRType(reflect.TypeOf(data)), transformer.Option{
		MIME: "xml",
	})
	...
	t.Run("EncodeTo", func(t *testing.T) {
		t.Run("raw value", func(t *testing.T) {
			...
			h := http.Header{}
			err := ct.EncodeTo(context.Background(), transformer.WriterWithHeader(b, h), data)
		...
		t.Run("reflect value", func(t *testing.T) {
			b := bytes.NewBuffer(nil)
			h := http.Header{}
			err := ct.EncodeTo(context.Background(), transformer.WriterWithHeader(b, h), reflect.ValueOf(data))
			NewWithT(t).Expect(err).To(BeNil())
			NewWithT(t).Expect(h.Get("Content-Type")).To(Equal("application/xml; charset=utf-8"))
		})
	})
	...
	t.Run("DecodeAndValidate", func(t *testing.T) {
		...
		t.Run("success", func(t *testing.T) {
			...
			err := ct.DecodeFrom(context.Background(), io.NopCloser(b), reflect.ValueOf(&data))
			NewWithT(t).Expect(err).To(BeNil())
		})
